...
def process_message(message, client):
    ...
    origin = splited[0]
    command = splited[2]
    sending_topic = "autopilotService2/" + origin  #autopilotService/miMain/telemetryInfo
    print ('autopiloto2 recibe ', command)
...
def AutopilotService (connection_mode, operation_mode, external_broker, username, password):
    ...
    internal_client = mqtt.Client("Autopilot_internal2")
    internal_client.on_message = on_internal_message
    internal_client.connect("localhost", 1884)


    external_client = mqtt.Client("Autopilot_external2_Adolfo", transport="websockets")
    ...
    print("Waiting....")
    external_client.subscribe("+/autopilotService2/#", 2)
    external_client.subscribe("cameraService/+/#", 2)
    internal_client.subscribe("+/autopilotService2/#")
